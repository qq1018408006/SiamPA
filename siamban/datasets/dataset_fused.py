...
class BANDataset(Dataset):
    def __init__(self,):
        ...
        self.pick = self.shuffle()
        self.points=Point(cfg.POINT.STRIDE, 15, cfg.TRAIN.EXEMPLAR_SIZE//2)
    ...
    def __getitem__(self, index):
        ...
        # augmentation
        template, template_box = self.template_aug(template_image,
                                        template_box,
                                        cfg.TRAIN.EXEMPLAR_SIZE,
                                        gray=gray)

        search, search_box = self.search_aug(search_image,
                                       search_box,
                                       cfg.TRAIN.SEARCH_SIZE,
                                       gray=gray)

        # get labels
        cls, delta = self.point_target(search_box, cfg.TRAIN.OUTPUT_SIZE, neg)
        template = template.transpose((2, 0, 1)).astype(np.float32)
        search = search.transpose((2, 0, 1)).astype(np.float32)
        batch_weight=np.zeros(1,dtype=np.int64) if neg else np.ones(1,np.int64)
        return {
                'template': template,
                'search': search,
                'label_cls': cls,#(b,25,25)
                'label_loc': delta,#(b,4,25,25)
                'search_bbox': np.array(search_box,dtype=np.float32),#(b,4)
                'template_bbox':np.array(template_box,dtype=np.float32),#(b,4)
                'batch_weight':batch_weight,#(b,1)
                }