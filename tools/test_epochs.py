...
parser.add_argument("--start_epoch", default=15, type=int, help="test end epoch")
parser.add_argument("--end_epoch", default=19, type=int,  help="test end epoch")
...
for i in range((args.end_epoch - args.start_epoch + 1) // args.threads + 1):
    ...
    os.system("python ../../tools/test.py --snapshot {0} --dataset {1} --config config1.yaml".format(snapshot, dataset))
