...
parser.add_argument('--config', default='config1.yaml', type=str,
        help='config file')
parser.add_argument('--snapshot', default='snapshot/checkpoint_e15.pth', type=str,
        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if args.vis and idx > 0:
                    ...
                    cv2.putText(img, str(idx), (40, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
                    #------------------------#
                    final_scores = outputs['final_scores'].reshape(25, 25)
                    final_scores=cv2.resize(final_scores, (250, 250), interpolation=cv2.INTER_LINEAR)
                    cv2.putText(final_scores, 'max=' + str(outputs['best_score']), (40, 120),
                                cv2.FONT_HERSHEY_SIMPLEX, 1,
                                (255, 0, 0), 2)
                    cv2.imshow(video.name + 'final_score', final_scores)
